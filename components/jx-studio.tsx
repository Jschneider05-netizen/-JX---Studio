...
function RequirementsPanel({ builder, setBuilder }: { builder: BuilderState; setBuilder: BuilderSetter }) {
  ...
  const updateRealEstate = (patch: Partial<RequirementsState["realEstate"]>) => update({ realEstate: { ...req.realEstate, ...patch } });
  const updateFitness = (patch: Partial<RequirementsState["fitness"]>) => update({ fitness: { ...req.fitness, ...patch } });
  const updateCommerce = (patch: Partial<RequirementsState["commerce"]>) => update({ commerce: { ...req.commerce, ...patch } });
  const updateAutomotive = (patch: Partial<RequirementsState["automotive"]>) => update({ automotive: { ...req.automotive, ...patch } });
  const updateServiceRequest = (patch: Partial<RequirementsState["serviceRequest"]>) => update({ serviceRequest: { ...req.serviceRequest, ...patch } });
  const updateVouchers = (patch: Partial<RequirementsState["vouchers"]>) => update({ vouchers: { ...req.vouchers, ...patch } });
  const updateNewsletter = (patch: Partial<RequirementsState["newsletter"]>) => update({ newsletter: { ...req.newsletter, ...patch } });
  ...
  const addProperty = () => updateRealEstate({ listings:[...req.realEstate.listings,{id:newId("property"),title:"Neues Objekt",type:"Haus",transaction:"sale",price:"",areaSqm:0,rooms:0,location:"",status:"available",imageUrl:"",exposeUrl:""}] });
  const addFitnessClass = () => updateFitness({ classes:[...req.fitness.classes,{id:newId("class"),name:"Neuer Kurs",trainer:"",schedule:"",durationMinutes:60,capacity:12,price:""}] });
  const addProduct = () => updateCommerce({ products:[...req.commerce.products,{id:newId("product"),name:"Neues Produkt",category:"",description:"",price:"",stock:0,variants:[]}] });
  const addAutoService = () => updateAutomotive({ services:[...req.automotive.services,{id:newId("auto-service"),name:"Neue Leistung",durationMinutes:60,price:""}] });
  const setAddon = (key:string, enabled:boolean) => setBuilder(prev=>({...prev,addons:enabled?[...new Set([...prev.addons,key])]:prev.addons.filter(x=>x!==key)}));
  return <div className="requirements-panel">
    ...
    {builder.industry==="Immobilien"&&<details><summary>Immobilien & Besichtigungen</summary><div className="requirement-body"><div className="switch-row"><div><b>Objektverwaltung aktivieren</b><small>Objekte, Filter und Besichtigungsanfragen vollständig mit dem Auftrag speichern.</small></div><Switch checked={req.realEstate.enabled} onCheckedChange={enabled=>{updateRealEstate({enabled});setAddon("realestate-listings",enabled)}}/></div>{req.realEstate.enabled&&<>
      <label className="input-label">Empfänger-E-Mail *<input type="email" value={req.realEstate.recipientEmail} onChange={e=>updateRealEstate({recipientEmail:e.target.value})}/></label>
      <div className="switch-row"><div><b>Objektfilter</b></div><Switch checked={req.realEstate.filters} onCheckedChange={filters=>updateRealEstate({filters})}/></div><div className="switch-row"><div><b>Besichtigungsanfrage</b></div><Switch checked={req.realEstate.viewingRequest} onCheckedChange={viewingRequest=>{updateRealEstate({viewingRequest});setAddon("realestate-viewings",viewingRequest)}}/></div>
      <div className="module-heading"><b>Objekte</b><button type="button" onClick={addProperty}><Plus size={13}/> Objekt</button></div>
      {req.realEstate.listings.map(item=><div className="menu-category-editor" key={item.id}><div className="requirement-row"><input placeholder="Objektname" value={item.title} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,title:e.target.value}:x)})}/><input placeholder="Preis" value={item.price} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,price:e.target.value}:x)})}/><button type="button" onClick={()=>updateRealEstate({listings:req.realEstate.listings.filter(x=>x.id!==item.id)})}><Trash2 size={14}/></button></div><div className="requirements-grid"><label>Objekttyp<input value={item.type} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,type:e.target.value}:x)})}/></label><label>Kauf / Miete<select value={item.transaction} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,transaction:e.target.value as typeof item.transaction}:x)})}><option value="sale">Kauf</option><option value="rent">Miete</option></select></label><label>Fläche m²<input type="number" min="0" value={item.areaSqm} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,areaSqm:Number(e.target.value)}:x)})}/></label><label>Zimmer<input type="number" min="0" step="0.5" value={item.rooms} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,rooms:Number(e.target.value)}:x)})}/></label><label>Ort / Lage<input value={item.location} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,location:e.target.value}:x)})}/></label><label>Status<select value={item.status} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,status:e.target.value as typeof item.status}:x)})}><option value="available">Verfügbar</option><option value="reserved">Reserviert</option><option value="sold">Verkauft</option><option value="rented">Vermietet</option></select></label><label>Bild-URL<input value={item.imageUrl} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,imageUrl:e.target.value}:x)})}/></label><label>Exposé-URL<input value={item.exposeUrl} onChange={e=>updateRealEstate({listings:req.realEstate.listings.map(x=>x.id===item.id?{...x,exposeUrl:e.target.value}:x)})}/></label></div></div>)}
      <label className="input-label">Objekte anzeigen<select value={req.realEstate.placement} onChange={e=>updateRealEstate({placement:e.target.value as typeof req.realEstate.placement})}><option value="page">Eigene/Leistungsseite</option><option value="home">Startseite</option><option value="both">Startseite + Seite</option></select></label>
    </>}</div></details>}

    {builder.industry==="Fitness"&&<details><summary>Kurse & Probetraining</summary><div className="requirement-body"><div className="switch-row"><div><b>Fitness-Modul aktivieren</b><small>Kurse, Trainer, Kapazität, Warteliste und Probetraining.</small></div><Switch checked={req.fitness.enabled} onCheckedChange={enabled=>{updateFitness({enabled});setAddon("fitness-classes",enabled)}}/></div>{req.fitness.enabled&&<>
      <label className="input-label">Empfänger-E-Mail *<input type="email" value={req.fitness.recipientEmail} onChange={e=>updateFitness({recipientEmail:e.target.value})}/></label><label className="input-label">Trainer (kommagetrennt)<input value={req.fitness.trainers.join(", ")} onChange={e=>updateFitness({trainers:e.target.value.split(",").map(v=>v.trim()).filter(Boolean)})}/></label>
      <div className="switch-row"><div><b>Probetraining anbieten</b></div><Switch checked={req.fitness.trialTraining} onCheckedChange={trialTraining=>{updateFitness({trialTraining});setAddon("fitness-trial",trialTraining)}}/></div><div className="switch-row"><div><b>Warteliste bei vollen Kursen</b></div><Switch checked={req.fitness.waitlist} onCheckedChange={waitlist=>updateFitness({waitlist})}/></div>
      <div className="module-heading"><b>Kurse</b><button type="button" onClick={addFitnessClass}><Plus size={13}/> Kurs</button></div>{req.fitness.classes.map(item=><div className="menu-category-editor" key={item.id}><div className="requirement-row"><input placeholder="Kurs" value={item.name} onChange={e=>updateFitness({classes:req.fitness.classes.map(x=>x.id===item.id?{...x,name:e.target.value}:x)})}/><input placeholder="Trainer" value={item.trainer} onChange={e=>updateFitness({classes:req.fitness.classes.map(x=>x.id===item.id?{...x,trainer:e.target.value}:x)})}/><button type="button" onClick={()=>updateFitness({classes:req.fitness.classes.filter(x=>x.id!==item.id)})}><Trash2 size={14}/></button></div><div className="requirements-grid"><label>Zeitplan *<input placeholder="Mo 18:00" value={item.schedule} onChange={e=>updateFitness({classes:req.fitness.classes.map(x=>x.id===item.id?{...x,schedule:e.target.value}:x)})}/></label><label>Dauer<input type="number" min="10" value={item.durationMinutes} onChange={e=>updateFitness({classes:req.fitness.classes.map(x=>x.id===item.id?{...x,durationMinutes:Number(e.target.value)}:x)})}/></label><label>Kapazität *<input type="number" min="1" value={item.capacity} onChange={e=>updateFitness({classes:req.fitness.classes.map(x=>x.id===item.id?{...x,capacity:Number(e.target.value)}:x)})}/></label><label>Preis<input value={item.price} onChange={e=>updateFitness({classes:req.fitness.classes.map(x=>x.id===item.id?{...x,price:e.target.value}:x)})}/></label></div></div>)}
    </>}</div></details>}

    {builder.industry==="Automotive"&&<details><summary>Werkstatt & Fahrzeuganfragen</summary><div className="requirement-body"><div className="switch-row"><div><b>Werkstatt-Modul aktivieren</b><small>Serviceleistungen, Fahrzeugdaten, Uploads und Anfragearten.</small></div><Switch checked={req.automotive.enabled} onCheckedChange={enabled=>{updateAutomotive({enabled});setAddon("auto-booking",enabled)}}/></div>{req.automotive.enabled&&<>
      <label className="input-label">Empfänger-E-Mail *<input type="email" value={req.automotive.recipientEmail} onChange={e=>updateAutomotive({recipientEmail:e.target.value})}/></label><label className="input-label">Anfragearten (kommagetrennt)<input value={req.automotive.requestTypes.join(", ")} onChange={e=>updateAutomotive({requestTypes:e.target.value.split(",").map(v=>v.trim()).filter(Boolean)})}/></label><label className="input-label">Fahrzeugdaten (kommagetrennt)<input value={req.automotive.vehicleFields.join(", ")} onChange={e=>updateAutomotive({vehicleFields:e.target.value.split(",").map(v=>v.trim()).filter(Boolean)})}/></label><label className="input-label">Einzugsgebiet<input placeholder="z. B. Bremen + 40 km" value={req.automotive.serviceArea} onChange={e=>updateAutomotive({serviceArea:e.target.value})}/></label><div className="switch-row"><div><b>Foto-/Datei-Uploads zulassen</b></div><Switch checked={req.automotive.allowUploads} onCheckedChange={allowUploads=>updateAutomotive({allowUploads})}/></div>
      <div className="module-heading"><b>Werkstatt-Leistungen</b><button type="button" onClick={addAutoService}><Plus size={13}/> Leistung</button></div>{req.automotive.services.map(item=><div className="requirement-row" key={item.id}><input value={item.name} onChange={e=>updateAutomotive({services:req.automotive.services.map(x=>x.id===item.id?{...x,name:e.target.value}:x)})}/><input type="number" min="5" value={item.durationMinutes} onChange={e=>updateAutomotive({services:req.automotive.services.map(x=>x.id===item.id?{...x,durationMinutes:Number(e.target.value)}:x)})}/><input placeholder="Preis" value={item.price} onChange={e=>updateAutomotive({services:req.automotive.services.map(x=>x.id===item.id?{...x,price:e.target.value}:x)})}/><button type="button" onClick={()=>updateAutomotive({services:req.automotive.services.filter(x=>x.id!==item.id)})}><Trash2 size={14}/></button></div>)}
    </>}</div></details>}

    {(builder.industry==="Handwerk"||builder.industry==="Reinigung")&&<details><summary>Projekt-/Leistungsanfrage</summary><div className="requirement-body"><div className="switch-row"><div><b>Anfrage-Konfigurator aktivieren</b><small>Leistung, Ort, Budget, Wunschzeitraum und Uploads strukturiert erfassen.</small></div><Switch checked={req.serviceRequest.enabled} onCheckedChange={enabled=>{updateServiceRequest({enabled});setAddon(builder.industry==="Reinigung"?"cleaning-request":"trade-estimator",enabled)}}/></div>{req.serviceRequest.enabled&&<><label className="input-label">Empfänger-E-Mail *<input type="email" value={req.serviceRequest.recipientEmail} onChange={e=>updateServiceRequest({recipientEmail:e.target.value})}/></label><label className="input-label">Auswählbare Leistungen *<input placeholder="Sanierung, Innenausbau, Reparatur" value={req.serviceRequest.serviceOptions.join(", ")} onChange={e=>updateServiceRequest({serviceOptions:e.target.value.split(",").map(v=>v.trim()).filter(Boolean)})}/></label><label className="input-label">Einzugsgebiet<input value={req.serviceRequest.serviceArea} onChange={e=>updateServiceRequest({serviceArea:e.target.value})}/></label><div className="switch-row"><div><b>Budget abfragen</b></div><Switch checked={req.serviceRequest.collectBudget} onCheckedChange={collectBudget=>updateServiceRequest({collectBudget})}/></div><div className="switch-row"><div><b>Wunschzeitraum abfragen</b></div><Switch checked={req.serviceRequest.collectPreferredDate} onCheckedChange={collectPreferredDate=>updateServiceRequest({collectPreferredDate})}/></div><div className="switch-row"><div><b>Foto-/Datei-Uploads</b></div><Switch checked={req.serviceRequest.allowUploads} onCheckedChange={allowUploads=>updateServiceRequest({allowUploads})}/></div></>}</div></details>}

    <details><summary>Shop & Commerce</summary><div className="requirement-body"><div className="switch-row"><div><b>Online-Shop aktivieren</b><small>Produkte und technische Shop-Anbindung vollständig festlegen.</small></div><Switch checked={req.commerce.enabled} onCheckedChange={enabled=>{updateCommerce({enabled});setAddon("shop",enabled)}}/></div>{req.commerce.enabled&&<>
      <div className="requirements-grid"><label>Shop-Provider<select value={req.commerce.provider} onChange={e=>updateCommerce({provider:e.target.value as typeof req.commerce.provider})}><option value="shopify">Shopify</option><option value="jx">JX Commerce</option><option value="external">Anderer Provider</option></select></label><label>Bestell-E-Mail *<input type="email" value={req.commerce.recipientEmail} onChange={e=>updateCommerce({recipientEmail:e.target.value})}/></label><label>Versand<select value={req.commerce.shippingMode} onChange={e=>updateCommerce({shippingMode:e.target.value as typeof req.commerce.shippingMode})}><option value="configured">Später detailliert konfigurieren</option><option value="flat">Pauschale</option><option value="pickup">Nur Abholung</option></select></label>{req.commerce.shippingMode==="flat"&&<label>Versandpreis<input value={req.commerce.shippingPrice} onChange={e=>updateCommerce({shippingPrice:e.target.value})}/></label>}</div><label className="input-label">Zahlungsarten (kommagetrennt)<input value={req.commerce.paymentMethods.join(", ")} onChange={e=>updateCommerce({paymentMethods:e.target.value.split(",").map(v=>v.trim()).filter(Boolean)})}/></label>
      <div className="module-heading"><b>Produkte</b><button type="button" onClick={addProduct}><Plus size={13}/> Produkt</button></div>{req.commerce.products.map(item=><div className="menu-category-editor" key={item.id}><div className="requirement-row"><input placeholder="Produktname" value={item.name} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,name:e.target.value}:x)})}/><input placeholder="Preis" value={item.price} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,price:e.target.value}:x)})}/><button type="button" onClick={()=>updateCommerce({products:req.commerce.products.filter(x=>x.id!==item.id)})}><Trash2 size={14}/></button></div><div className="requirements-grid"><label>Kategorie<input value={item.category} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,category:e.target.value}:x)})}/></label><label>Bestand<input type="number" min="0" value={item.stock} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,stock:Number(e.target.value)}:x)})}/></label><label className="full">Beschreibung<textarea rows={2} value={item.description} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,description:e.target.value}:x)})}/></label></div><div className="module-heading"><b>Varianten</b><button type="button" onClick={()=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,variants:[...x.variants,{id:newId("variant"),name:"Neue Variante",price:item.price,stock:0}]}:x)})}><Plus size={13}/> Variante</button></div>{item.variants.map(variant=><div className="requirement-row" key={variant.id}><input placeholder="z. B. Größe M" value={variant.name} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,variants:x.variants.map(v=>v.id===variant.id?{...v,name:e.target.value}:v)}:x)})}/><input placeholder="Preis" value={variant.price} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,variants:x.variants.map(v=>v.id===variant.id?{...v,price:e.target.value}:v)}:x)})}/><input aria-label="Variantenbestand" type="number" min="0" value={variant.stock} onChange={e=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,variants:x.variants.map(v=>v.id===variant.id?{...v,stock:Number(e.target.value)}:v)}:x)})}/><button type="button" onClick={()=>updateCommerce({products:req.commerce.products.map(x=>x.id===item.id?{...x,variants:x.variants.filter(v=>v.id!==variant.id)}:x)})}><Trash2 size={14}/></button></div>)}</div>)}
    </>}</div></details>

    <details><summary>Gutscheine & Newsletter</summary><div className="requirement-body"><div className="switch-row"><div><b>Gutscheine</b><small>Werte und Gültigkeit für einen späteren Gutschein-Checkout.</small></div><Switch checked={req.vouchers.enabled} onCheckedChange={enabled=>{updateVouchers({enabled});setAddon("vouchers",enabled)}}/></div>{req.vouchers.enabled&&<div className="requirements-grid"><label>Empfänger-E-Mail *<input type="email" value={req.vouchers.recipientEmail} onChange={e=>updateVouchers({recipientEmail:e.target.value})}/></label><label>Werte in €<input value={req.vouchers.presetValues.join(", ")} onChange={e=>updateVouchers({presetValues:e.target.value.split(",").map(v=>Number(v.trim())).filter(v=>Number.isFinite(v)&&v>0)})}/></label><label>Gültigkeit Monate<input type="number" min="1" value={req.vouchers.validityMonths} onChange={e=>updateVouchers({validityMonths:Number(e.target.value)})}/></label><label><span>Freier Betrag</span><Switch checked={req.vouchers.customAmount} onCheckedChange={customAmount=>updateVouchers({customAmount})}/></label></div>}
      <div className="switch-row"><div><b>Newsletter</b><small>Provider, Liste und Double-Opt-in festlegen.</small></div><Switch checked={req.newsletter.enabled} onCheckedChange={enabled=>{updateNewsletter({enabled});setAddon("newsletter",enabled)}}/></div>{req.newsletter.enabled&&<div className="requirements-grid"><label>Provider *<input placeholder="z. B. Brevo / Mailchimp" value={req.newsletter.provider} onChange={e=>updateNewsletter({provider:e.target.value})}/></label><label>Liste *<input value={req.newsletter.listName} onChange={e=>updateNewsletter({listName:e.target.value})}/></label><label className="full">Erfolgsmeldung<input value={req.newsletter.successMessage} onChange={e=>updateNewsletter({successMessage:e.target.value})}/></label><label><span>Double-Opt-in</span><Switch checked={req.newsletter.doubleOptIn} onCheckedChange={doubleOptIn=>updateNewsletter({doubleOptIn})}/></label></div>}</div></details>
      ...
      <aside className={`builder-sidebar ${toolsOpen ? "mobile-open" : ""}`}>
        ...
        <Tabs value={tab} onValueChange={setTab} className="builder-tabs">
          ...
          <TabsContent value="features" className="tab-panel">
            ...
            <div className="industry-module-box"><div className="module-heading"><b>{builder.industry} Funktionen</b><small>Branchenspezifische Logik, nicht nur Design.</small></div>{(industryModules[builder.industry] ?? []).map((module) => <label className="industry-module" key={module.key}><Checkbox checked={builder.addons.includes(module.key)} onCheckedChange={(checked) => setBuilder((prev) => { const requirements=normalizeRequirements(prev.requirements); const addons=checked?[...new Set([...prev.addons,module.key])]:prev.addons.filter(item=>item!==module.key); const appointmentKeys=new Set(["beauty-booking","practice-appointments","tattoo-consultation","restaurant-reservations"]); const realEstateKeys=new Set(["realestate-listings","realestate-viewings"]); const fitnessKeys=new Set(["fitness-classes","fitness-trial"]); const automotiveKeys=new Set(["auto-booking","auto-inventory"]); const serviceRequestKeys=new Set(["trade-estimator","cleaning-request"]); return {...prev,addons,requirements:{...requirements,appointments:appointmentKeys.has(module.key)?{...requirements.appointments,enabled:Boolean(checked)}:requirements.appointments,restaurantMenu:module.key==="restaurant-menu"?{...requirements.restaurantMenu,enabled:Boolean(checked)}:requirements.restaurantMenu,realEstate:realEstateKeys.has(module.key)?{...requirements.realEstate,enabled:addons.some(key=>realEstateKeys.has(key))}:requirements.realEstate,fitness:fitnessKeys.has(module.key)?{...requirements.fitness,enabled:addons.some(key=>fitnessKeys.has(key))}:requirements.fitness,automotive:automotiveKeys.has(module.key)?{...requirements.automotive,enabled:addons.some(key=>automotiveKeys.has(key))}:requirements.automotive,serviceRequest:serviceRequestKeys.has(module.key)?{...requirements.serviceRequest,enabled:addons.some(key=>serviceRequestKeys.has(key))}:requirements.serviceRequest}}; })}/><span><b>{module.title}</b><small>{module.copy}</small></span></label>)}</div>
            <div className="addon-list">{Object.entries(addonLabels).map(([key, label]) => <label key={key}><Checkbox checked={builder.addons.includes(key)} onCheckedChange={(checked) => setBuilder((prev) => { const requirements=normalizeRequirements(prev.requirements); return { ...prev, addons: checked ? [...new Set([...prev.addons, key])] : prev.addons.filter((item) => item !== key), requirements: key==="booking" ? { ...requirements, appointments:{...requirements.appointments,enabled:Boolean(checked)} } : key==="shop" ? { ...requirements, commerce:{...requirements.commerce,enabled:Boolean(checked)} } : requirements }; })} /><span><b>{label}</b><small>+ {money(addonPrices[key])}</small></span></label>)}</div>
...
function ConfiguredBusinessModules({ builder, page }: { builder: BuilderState; page: PageConfig }) {
  const req=normalizeRequirements(builder.requirements);
  const onHome=page.id==="home"; const onContact=page.id==="contact"; const onServices=page.id==="services";
  ...
  const menuVisible=req.restaurantMenu.enabled && ((req.restaurantMenu.placement==="home"&&onHome)||(req.restaurantMenu.placement==="both"&&(onHome||onServices))||(req.restaurantMenu.placement==="page"&&onServices));
  ...
  const realEstateVisible=req.realEstate.enabled&&visible(req.realEstate.placement);
  const fitnessVisible=req.fitness.enabled&&visible(req.fitness.placement);
  const commerceVisible=req.commerce.enabled&&visible(req.commerce.placement);
  const automotiveVisible=req.automotive.enabled&&visible(req.automotive.placement);
  const serviceRequestVisible=req.serviceRequest.enabled&&visible(req.serviceRequest.placement);
  const voucherVisible=req.vouchers.enabled&&onHome;
  const newsletterVisible=req.newsletter.enabled&&(onHome||onContact);
  if(!menuVisible&&!bookingVisible&&!contactVisible&&!realEstateVisible&&!fitnessVisible&&!commerceVisible&&!automotiveVisible&&!serviceRequestVisible&&!voucherVisible&&!newsletterVisible)return null;
  return <>
    ...
    {realEstateVisible&&<section className="pv-business-module pv-booking-module"><small>IMMOBILIEN</small><h2>Aktuelle Objekte</h2><div className="pv-booking-grid">{req.realEstate.listings.map(item=><article key={item.id}><div><b>{item.title}</b><small>{item.location||item.type} · {item.areaSqm||"–"} m² · {item.rooms||"–"} Zimmer</small></div><strong>{item.price}</strong></article>)}</div>{req.realEstate.viewingRequest&&<button type="button">Besichtigung anfragen</button>}</section>}
    {fitnessVisible&&<section className="pv-business-module pv-booking-module"><small>KURSE</small><h2>Training, das in deinen Alltag passt</h2><div className="pv-booking-grid">{req.fitness.classes.map(item=><article key={item.id}><div><b>{item.name}</b><small>{item.trainer||"Trainer folgt"} · {item.schedule} · {item.durationMinutes} Min.</small></div><strong>{item.capacity} Plätze</strong></article>)}</div>{req.fitness.trialTraining&&<button type="button">Probetraining anfragen</button>}</section>}
    {commerceVisible&&<section className="pv-business-module pv-booking-module"><small>SHOP</small><h2>Produkte</h2><div className="pv-booking-grid">{req.commerce.products.map(item=><article key={item.id}><div><b>{item.name}</b><small>{item.category||"Produkt"}{item.description?` · ${item.description}`:""}</small></div><strong>{item.price}</strong></article>)}</div><small>Commerce Provider: {req.commerce.provider}</small></section>}
    {automotiveVisible&&<section className="pv-business-module pv-booking-module"><small>WERKSTATT</small><h2>Service anfragen</h2><div className="pv-booking-grid">{req.automotive.services.map(item=><article key={item.id}><div><b>{item.name}</b><small>{item.durationMinutes} Min.{item.price?` · ${item.price}`:""}</small></div><button type="button">Anfragen</button></article>)}</div>{req.automotive.serviceArea&&<p>Einzugsgebiet: {req.automotive.serviceArea}</p>}</section>}
    {serviceRequestVisible&&<section className="pv-business-module pv-form-module"><small>PROJEKTANFRAGE</small><h2>Was können wir für dich umsetzen?</h2><div className="pv-form-preview"><label>Leistung<select disabled><option>{req.serviceRequest.serviceOptions[0]||"Leistung auswählen"}</option></select></label><label>Ort<input disabled /></label>{req.serviceRequest.collectBudget&&<label>Budget<input disabled /></label>}{req.serviceRequest.collectPreferredDate&&<label>Wunschzeitraum<input disabled /></label>}<button type="button">Projekt anfragen</button></div></section>}
    {voucherVisible&&<section className="pv-business-module pv-booking-module"><small>GUTSCHEINE</small><h2>Freude verschenken</h2><div className="pv-booking-grid">{req.vouchers.presetValues.map(value=><article key={value}><div><b>{value} € Gutschein</b><small>{req.vouchers.validityMonths} Monate gültig</small></div><button type="button">Auswählen</button></article>)}</div></section>}
    {newsletterVisible&&<section className="pv-business-module pv-form-module"><small>NEWSLETTER</small><h2>Neuigkeiten direkt per E-Mail</h2><div className="pv-form-preview"><label>E-Mail<input disabled /></label><button type="button">Anmelden</button>{req.newsletter.doubleOptIn&&<small>Double-Opt-in aktiviert</small>}</div></section>}
