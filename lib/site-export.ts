...
export async function buildSiteFiles(c:BuilderState,id:string):Promise<Record<string,string>> {
  ...
  const renderConfiguredModules = (page: PageConfig) => {
    const onHome=page.id==="home", onContact=page.id==="contact";
    const menuVisible=requirements.restaurantMenu.enabled && ((requirements.restaurantMenu.placement==="home"&&onHome)||(requirements.restaurantMenu.placement==="both"&&(onHome||page.id==="services"))||(requirements.restaurantMenu.placement==="page"&&page.id==="services"));
